...
class Timer(ABC):
    ...
    def __init__(self, electronic_name):
        self.electronic_name = electronic_name
    
    def timer(self):
        time_len: int = self.set_time()
        self.start_time(time_len)
        self.end_time()

    def set_time(self) -> int:
        while True:
            try:
                timer_length = int(input("Please insert a time to track: "))
    ...
    @abstractmethod
    def start_time(self, t_len):
        raise NotImplementedError("Define an electronic to use.")
...
class Clock(Timer):
    ...
    def __init__(self, electronic_name):
        super().__init__(electronic_name)
...
# Implement the Composite Patter!
from abc import ABC, abstractmethod
...
class File(FileSystemComponent):
    ...
    def display(self, indent=0):
        ...
        indentation = " " * indent
        return indentation + f"File: {self.name} ({self.get_size()} KB)"
...
class FileSystem:
    ...
    def _get_directory_from_path(self, path):
        ...
        for i in range(len(path_parts)):
            ...
            if not component:
                raise ValueError(f"Path not fount: {path}")
